...
#define DEFAULT_FILE_BUF_SIZE (2 * 1024 * 1024)
...
#define client_send_ctrl_msg(cl, str) \
	sceNetSend(cl->ctrl_sockfd, str, strnlen(str, PATH_MAX), 0)
...
static inline void client_send_data_msg(ftpvita_client_info_t *client, const char *str)
{
	if (client->data_con_type == FTP_DATA_CONNECTION_ACTIVE) {
		sceNetSend(client->data_sockfd, str, strnlen(str, PATH_MAX), 0);
	} else {
		sceNetSend(client->pasv_sockfd, str, strnlen(str, PATH_MAX), 0);
	}
}
...
static void cmd_PASV_func(ftpvita_client_info_t *client)
{
	...
	snprintf(data_socket_name, 64, "FTPVita_client_%i_data_socket",
		client->num);
	...
	/* Build the command */
	snprintf(cmd, 512, "227 Entering Passive Mode (%hhu,%hhu,%hhu,%hhu,%hhu,%hhu)" FTPVITA_EOL,
...
{
	char msg[PATH_MAX];
	snprintf(msg, PATH_MAX, "257 \"%s\" is the current directory." FTPVITA_EOL, client->cur_path);
	client_send_ctrl_msg(client, msg);
}

static int path_is_at_root(const char *path)
{
	return strrchr(path, '/') == (path + strnlen(path, PATH_MAX) - 1);
}

static void dir_up(char *path)
{
	char *pch;
	size_t len_in = strnlen(path, PATH_MAX);
	if (len_in == 1) {
...
static void cmd_CWD_func(ftpvita_client_info_t *client)
{
	...
	if (n < 1) {
		...
		if (strcmp(cmd_path, "/") == 0) {
			strncpy(client->cur_path, cmd_path, PATH_MAX);
		} else  if (strcmp(cmd_path, "..") == 0) {
			dir_up(client->cur_path);
		} else {
			if (cmd_path[0] == '/') { /* Full path */
				strncpy(tmp_path, cmd_path, PATH_MAX);
				...
				if (path_is_at_root(client->cur_path))
					snprintf(tmp_path, PATH_MAX, "%s%s", client->cur_path, cmd_path);
				else
					snprintf(tmp_path, PATH_MAX, "%s/%s", client->cur_path, cmd_path);
			}
			...
			strncpy(client->cur_path, tmp_path, PATH_MAX);
		}
...
static void cmd_TYPE_func(ftpvita_client_info_t *client)
{
	...
	n_args = sscanf(client->recv_buffer, "%*s %c %s", &data_type, format_control);
...
static void gen_filepath(ftpvita_client_info_t *client, char *dest_path)
{
	...
	if (cmd_path[0] == '/') {
		/* Full path */
		strncpy(dest_path, cmd_path, PATH_MAX);
	} else {
		/* The file is relative to current dir, so
		 * append the file to the current path */
		snprintf(dest_path, PATH_MAX, "%s/%s", client->cur_path, cmd_path);
	}
}
...
static void cmd_RNFR_func(ftpvita_client_info_t *client)
{
	...
	strncpy(client->rename_path, vita_path_src, PATH_MAX);
...
static void cmd_SIZE_func(ftpvita_client_info_t *client)
{
	...
	snprintf(cmd, 64, "213 %lld" FTPVITA_EOL, stat.st_size);
	client_send_ctrl_msg(client, cmd);
}
...
static void cmd_REST_func(ftpvita_client_info_t *client)
{
	...
	snprintf(cmd, 64, "350 Resuming at %d" FTPVITA_EOL, client->restore_point);
	client_send_ctrl_msg(client, cmd);
}
	...
	while (1) {
		...
		if (client->n_recv > 0) {
			...
			if ((dispatch_func = get_dispatch_func(cmd))) {
				...
				DEBUG("Command not implemented: %s. len: %d / %d", cmd, strnlen(cmd,16));
...
static int server_thread(SceSize args, void *argp)
{
	...
	while (1) {
		...
		if (client_sockfd >= 0) {
			...
			snprintf(client_thread_name, 64, "FTPVita_client_%i_thread",
				number_clients);
			...
			client->data_con_type = FTP_DATA_CONNECTION_NONE;
			strncpy(client->cur_path, FTP_DEFAULT_PATH, PATH_MAX);
...
int ftpvita_init(char *vita_ip, unsigned short int *vita_port)
{
	...
	/* Return data */
	strncpy(vita_ip, info.ip_address, 16);
	*vita_port = FTP_PORT;
...
int ftpvita_add_device(const char *devname)
{
	...
	for (i = 0; i < MAX_DEVICES; i++) {
		if (!device_list[i].valid) {
			strncpy(device_list[i].name, devname, PATH_MAX);
