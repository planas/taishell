...
int sceClibTolower(int c);
void * sceClibMemchr(const void * ptr, int value, size_t num);
long long int sceClibStrtoll(const char* str, char** endptr, int base);

#define tolower sceClibTolower
#define memchr  sceClibMemchr
#define strtol  sceClibStrtoll
...
static int GET(struct status_t * status) {
  int rc = (*status->s == '\0') ? EOF : (unsigned char)*((status->s)++);
...
*/
static void UNGET( int c, struct status_t * status ) {
  --(status->s);
  --(status->i);
  --(status->current);
}
...
static const char * scan(const char * spec, struct status_t * status) {
    ...
    case 's':
    {
      ...
      while ((status->current < status->width) && ((rc = GET(status)) != EOF)) {
        if (isspace(rc)) {
          ...
          if (value_parsed) { /* matching sequence terminated by whitespace */
            if (!(status->flags & E_suppressed)) {
            	*c = '\0';
            }
            ++status->n;
  ...
  if (status->base != -1) {
    ...
    while ((status->current < status->width ) && ((rc = GET(status)) != EOF)) {
      if (isspace(rc)) {
        ...
        char * digitptr = memchr(digits, tolower(rc), status->base);
